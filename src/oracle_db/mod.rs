            ...
            Some(tx.prepare(
                ...
                INSERT OR IGNORE INTO oracle_ticks
                    (ts, symbol, market_price, chainlink_price, chainlink_age_secs,
                     ...
                     latency_binance_ms, latency_coinbase_ms, latency_kraken_ms,
                     ema_12, ema_26, ema_50, rsi_14,
                     momentum_10, momentum_20, volatility,
                     bb_upper, bb_middle, bb_lower,
                     macd, macd_signal, macd_histogram,
                     p_lmsr, delta_lmsr, delta_lmsr_z, lmsr_version, alpha_live, b_live)
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
                "#,
            )?)
        } else {
                ...
                WsEvent::Tick { ts, data } => {
                    if let Some(tick_stmt) = tick_stmt.as_mut() {
                        tick_stmt.execute(params![
                            ...
                            data.exchange_latency_ms.as_ref().and_then(|m| m.get("binance").copied()),
                            data.exchange_latency_ms.as_ref().and_then(|m| m.get("coinbase").copied()),
                            data.exchange_latency_ms.as_ref().and_then(|m| m.get("kraken").copied()),
